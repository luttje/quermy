 ...
 * CHANGE COLUMN, incoming foreign keys, index management, FK management,
 * column reordering).
 */
...
/*
 * MySQL-specific: incoming foreign keys
 */
...
it('incoming FK entries have all required keys', function () {
    ...
    $this->driver->runQuery($this->database, 'CREATE TABLE `fk_shape_parent_in` (id INT PRIMARY KEY)');
    $this->driver->runQuery(
        $this->database,
...
/*
 * MySQL-specific: ADD COLUMN … AFTER
 */
...
/*
 * MySQL-specific: CHANGE COLUMN (rename + retype)
 */
...
/*
 * MySQL-specific: PRIMARY KEY management
 */
it('creates and drops a primary key constraint', function () {
    ...
    $this->driver->runQuery(
        $this->database,
        'CREATE TABLE `pk_mgmt_probe` (id INT NOT NULL, name VARCHAR(50))'
    );
...
/*
 * MySQL-specific: getDatabaseInfo returns charset and collation
 */
it('getDatabaseInfo returns a non-null charset and collation for MySQL', function () {
...
/*
 * MySQL-specific: JSON round-trip
 */
