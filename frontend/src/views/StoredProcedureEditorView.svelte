<script>
    ...
    function buildTemplateSql(name) {
        const ioOpen = capabilities?.identifierOpen ?? "`";
        const n = name || "procedure_name";
        if (ioOpen === '"') {
            return `CREATE OR REPLACE PROCEDURE public.${quoteIdent(n)}()\nLANGUAGE plpgsql\nAS $$\nBEGIN\n  -- procedure body\nEND;\n$$;`;
        }
        if (ioOpen === "[") {
            return `CREATE PROCEDURE [dbo].${quoteIdent(n)}\nAS\nBEGIN\n  -- procedure body\nEND`;
        }
        return `CREATE PROCEDURE ${quoteIdent(n)}()\nBEGIN\n  -- procedure body\nEND`;
    }
    ...
    async function loadItems() {
        ...
        try {
            const r = await api.listProcedures(sourceDb);
            const list = r.procedures || [];
            const defs = await Promise.all(
                list.map(async (name) => {
                    try {
                        const res = await api.getProcedureDefinition(
                            sourceDb,
                            name,
                        );
    ...
    async function createItem() {
        ...
        if (!targetName) {
            toast("Procedure name is required", "error");
            return;
        }
        if (!sql) {
            toast("Procedure SQL is required", "error");
            return;
        }
        ...
        savingCreate = true;
        try {
            await api.saveProcedureDefinition(db, targetName, createDefinition);
            toast(
                existed
                    ? `Procedure "${targetName}" updated`
                    : `Procedure "${targetName}" created`,
                "success",
            );
            showCreateModal = false;
            await loadItems();
    ...
    async function openEditModal(name) {
        ...
        try {
            const r = await api.getProcedureDefinition(db, name);
    ...
    async function saveEdited() {
        ...
        savingEdit = true;
        try {
            await api.saveProcedureDefinition(db, editName, editDefinition);
            definitions = { ...definitions, [editName]: editDefinition };
            toast(`Procedure "${editName}" saved`, "success");
            showEditModal = false;
            await loadItems();
        } catch (e) {
            toast(e.message, "error");
    ...
    async function confirmDelete() {
        ...
        dropping = true;
        try {
            await api.dropProcedure(db, deleteName);
            toast(`Dropped procedure "${deleteName}"`, "success");
            showDeleteModal = false;
            await loadItems();
        } catch (e) {
            toast(e.message, "error");
    ...
    >
        <div class="flex flex-col">
            ...
            <p class="mono text-(--ink-3) text-[10.5px]">
                Browse, edit, and delete stored procedures.
            </p>
        </div>
    ...
    <div class="p-2.5">
        ...
        {:else if items.length === 0}
            <div
                class="px-2 py-4 text-center mono text-(--ink-3) text-[11.5px]"
            >
                No stored procedures found in {db}.
            </div>
        {:else}
...
>
    ...
    <div slot="footer" class="px-5 py-3 bg-(--bg-2) flex justify-end gap-2">
        ...
        <Btn
            ...
            on:click={createItem}
        >
            {savingCreate ? "Creating…" : "Create Procedure"}
        </Btn>
    </div>
</Modal>
...
>
    ...
    <div slot="footer" class="px-5 py-3 bg-(--bg-2) flex justify-end gap-2">
        ...
        <Btn
            ...
            on:click={saveEdited}
        >
            {savingEdit ? "Saving…" : "Save Procedure"}
        </Btn>
    </div>
</Modal>
...
<Modal
    ...
    title={`Delete Procedure · ${deleteName}`}
    maxWidth="max-w-md"
...
>
    ...
    <div slot="footer" class="px-5 py-3 bg-(--bg-2) flex justify-end gap-2">
        ...
        >
            {dropping ? "Deleting…" : "Delete Procedure"}
        </Btn>
    </div>
</Modal>
